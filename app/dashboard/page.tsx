  ...
  useEffect(() => {
    const init = async () => {
      ...
      if (profile.role !== 'super_admin') {
        ...
        const { data: att } = await supabase.from('attendance').select('*').eq('user_id', session.user.id).eq('tanggal', today).single()
      ...
      <div style={{ background: C.white, borderBottom: `1px solid ${C.border}`, padding: '12px 20px', display: 'flex', alignItems: 'center', justifyContent: 'space-between', position: 'sticky', top: 0, zIndex: 10 }}>
        <div style={{ display: 'flex', alignItems: 'center', gap: 10 }}>
          ...
          <div>
            ...
            <div style={{ fontSize: 10, color: C.textMuted }}>{user.full_name} {isAdmin ? '— Owner' : `— ${user.divisi}`}</div>
          </div>
        </div>
        <button onClick={handleLogout} style={{ background: C.surfaceAlt, border: 'none', borderRadius: 8, padding: '6px 12px', fontSize: 12, color: C.textMuted, cursor: 'pointer', fontWeight: 600 }}>Keluar</button>
      </div>
...
function PoinScreen({ user }: { user: Profile }) {
  ...
  useEffect(() => {
    const load = async () => {
      const { data } = await supabase.from('attendance').select('*').eq('user_id', user.id).eq('status', 'hadir').order('tanggal', { ascending: false }).limit(30)
      if (data) {
        setHistory(data)
    ...
    <div style={{ display: 'flex', flexDirection: 'column', gap: 16 }}>
      <div style={{ background: `linear-gradient(135deg, ${C.primary}, ${C.primaryDark})`, borderRadius: 16, padding: '28px 20px', textAlign: 'center' }}>
        ...
        <div style={{ marginTop: 14, display: 'inline-block', background: 'rgba(255,255,255,0.15)', padding: '6px 16px', borderRadius: 20 }}>
          <span style={{ color: '#fff', fontSize: 13, fontWeight: 700 }}>{total >= 100 ? '🥇 Excellent' : total >= 60 ? '🥈 Good' : '🥉 Keep it up!'}</span>
        </div>
      </div>
      ...
      <div style={{ background: C.white, borderRadius: 16, padding: 20, boxShadow: '0 2px 12px rgba(27,94,58,0.08)', border: `1px solid ${C.border}` }}>
        ...
        <div style={{ display: 'flex', flexDirection: 'column', gap: 10 }}>
          {history.length === 0 ? <div style={{ textAlign: 'center', color: C.textMuted, padding: '16px 0' }}>Belum ada riwayat kehadiran</div> : history.map((att, i) => (
